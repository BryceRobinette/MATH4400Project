...
# Now we need to figure out why our data is giving us grief
...
line.fit = lm(outcome~ dumbvar1+dumbvar2, data = df, subset = -test.index)
summary(line.fit)
alias(line.fit)
...
#-------------------------------------------------------------------------------------------
lda.fit = lda(outcome~., data = df, subset = -test.index)
summary(lda.fit)

lda.pred = predict(lda.fit, df[test.index, ])
lda.mean = mean(lda.pred$class == df[test.index,'outcome'])
table(lda.pred$class == df[test.index, 'outcome'])
lda.means
